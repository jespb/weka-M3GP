...
public class ReadJSON5 {
	...
	public static void main(String [] args) throws IOException{
		...
		for(int g = 0; g < gens; g++) {
			...
			out.write(matrixToR("pop_goa_"+(g), goa2[g]));
			out.write("\n");
		...
		out.write("\n\n");
		for (int g = 0; g < gens; g++) {
			out.write("for (i in 1:"+goa2[1][0].length+") {pop_goa_"+g+"[1,i] <- mean(pop_goa_"+g+"[,i])}\n");
			out.write("x <- sum(pop_goa_"+g+"[1,])\n");
			out.write("for (i in 1:"+goa2[1][0].length+") {pop_goa_"+g+"[1,i] <- pop_goa_"+g+"[1,i]/x}\n");
		}
		
		out.write("fin <- matrix( ncol = 99, nrow = "+ goa2[1][0].length +")\n");
		
		for(int g = 0; g < gens; g++) {
			for(int go = 1 ; go < goa2[1][0].length+1; go++) {
		...
		System.out.println(goa2[1][0].length);
