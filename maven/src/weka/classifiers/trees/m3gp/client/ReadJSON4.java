...
public class ReadJSON4 {
	...
	public static void main(String [] args) throws IOException{
		...
		for(int g = 1; g < gens; g++) {
			out.write("# "+(g+1)+"th generation: Average probability of the populationl\n");
			out.write(matrixToR("pop_goa_"+(g), goa1[g]));
			out.write("\n");
		...
		out.write("\n\n");
		for (int g = 1; g < gens; g++) {
			out.write("for (i in 1:"+goa1[1][0].length+") {pop_goa_"+g+"[1,i] <- mean(pop_goa_"+g+"[,i])}\n");
			out.write("x <- sum(pop_goa_"+g+"[1,])\n");
			out.write("for (i in 1:"+goa1[1][0].length+") {pop_goa_"+g+"[1,i] <- pop_goa_"+g+"[1,i]/x}\n");
		}
		
		out.write("fin <- matrix( ncol = 99, nrow = "+ goa1[1][0].length +")\n");
		
		for(int g = 1; g < gens; g++) {
			for(int go = 1 ; go < goa1[1][0].length+1; go++) {
				out.write("fin["+go+","+g+"] <- pop_goa_"+(g)+"[1,"+go+"]\n");
			}
		}
		
		out.write("\n\n");
		
		System.out.println(goa1[1][0].length);
